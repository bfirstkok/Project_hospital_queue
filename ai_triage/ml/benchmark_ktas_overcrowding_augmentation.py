...
def sample_external(
    ...
    *,
    ...
    for label in LABELS:
        ...
        parts.append(group)
    if not parts:
        raise ValueError("No valid external KTAS rows available")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate augmentation with the public 73,776-row Korean KTAS "
            "vital-sign cohort. Validation remains project-only."
        )
    )
    ...
    parser.add_argument(
        "--weights",
        default="0.01,0.025,0.05,0.10,0.25,0.50",
    )
    ...
    feature_coverage = {
        feature: int(external[feature].notna().sum())
        if feature != "chief_complain"
        else int(external[feature].fillna("").astype(str).str.strip().ne("").sum())
        for feature in features
    }
    ...
    summary = {
        ...
        "external_runtime_feature_non_null": feature_coverage,
    ...
    lines = [
        "# 73,776-row public KTAS vital-sign augmentation benchmark",
        "",
        "Source: PLOS ONE DOI 10.1371/journal.pone.0247042, S1 Data.",
        "",
        "The external cohort contains direct KTAS levels plus age, blood pressure, "
        "pulse rate, respiratory rate, body temperature, SpO2, and mental state. "
        "All validation rows remain from the project's original dataset.",
        "",
        ...
        "",
        "| External weight | Accuracy | Balanced Acc. | Macro F1 | RED Recall | PINK Recall | Route Acc. | Severe under-triage | ΔRED | ΔPINK |",
        "|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    lines += [
        ...
        "## Leakage controls",
        "",
        "- Admission, ED overcrowding, and patient-count variables are excluded.",
        "- Only features already observable by the deployed model are used.",
        "- Complaint category is not converted into fake free text.",
        "- Production is unchanged unless project-only validation improves safely.",
    ]
