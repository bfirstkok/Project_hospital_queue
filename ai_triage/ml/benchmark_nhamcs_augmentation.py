...
def sample_external(
    ...
    *,
    ...
    for label in LABELS:
        ...
        parts.append(group)
    if not parts:
        raise ValueError("No valid NHAMCS rows available")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate whether NHAMCS augmentation improves the project-domain "
            "Random Forest. All reported validation folds are project-only."
        )
    )
    parser.add_argument("--nhamcs-csv", required=True)
    ...
    parser.add_argument(
        "--weights",
        default="0.10,0.25,0.35,0.50,1.00",
        help="Comma-separated external sample weights.",
    )
    ...
    external = pd.read_csv(args.nhamcs_csv, low_memory=False)
    ...
    for weight in weights:
        ...
        print(f"Running external_weight={weight:.2f}", flush=True)
        results.append(
            evaluate_weight(
                project,
                external,
    ...
    summary = {
        ...
        "nhamcs_rows_available": int(full_external_rows),
        "nhamcs_rows_used": int(len(external)),
        "nhamcs_class_counts_used": {
    ...
    lines = [
        "# NHAMCS 2018-2022 augmentation benchmark",
        "",
        "NHAMCS public-use ED data are used only as external training augmentation.",
        "All validation rows come from the project's original dataset.",
        "",
        f"- Project rows: {len(project)}",
        f"- NHAMCS rows available after conversion: {full_external_rows}",
        f"- NHAMCS rows used after class cap: {len(external)}",
        "- NHAMCS class counts used: "
        + ", ".join(
        ...
        f"- Balanced accuracy: {pct(baseline['balanced_accuracy'])}",
        ...
        "",
        "| NHAMCS weight | Accuracy | Macro F1 | RED Recall | PINK Recall | Route Acc. | Severe under-triage | ΔRED | ΔPINK |",
        "|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for result in results:
    ...
    lines += [
        ...
        "## Interpretation rule",
        "",
        "Do not replace the production model merely because one metric increases. "
        "A candidate should improve high-acuity recall without materially worsening "
        "route accuracy or severe under-triage on project-only validation.",
        "",
        "NHAMCS IMMEDR levels are harmonized to the project's five colors for an "
        "external-data experiment; they are not claimed to be clinically identical "
        "to KTAS.",
    ]
